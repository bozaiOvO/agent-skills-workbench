---
name: darwin-skill
description: "自动评测并迭代优化 Skill。用于 Skill 打分、质量检查、自动优化和回归验证。"
...
模板位置：`templates/result-card.html`（**如果模板文件不存在，跳过卡片生成步骤，直接以文本格式输出分数变化和改进摘要**）
...
```
1. 检查 templates/result-card.html 是否存在。如果不存在，跳过步骤2-4，直接以 markdown 表格输出分数变化。如果存在，复制到临时工作文件
2. 用 sed/编辑工具 替换占位数据：
