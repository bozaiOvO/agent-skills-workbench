---
name: huashu-nuwa
description: "深度调研人物或主题，并生成可运行的人物视角 Skill。用于造 Skill、蒸馏人物思维方式。"
...
**触发条件**：用户提供的本地素材是大量直播转录/播客转录/视频ASR文本（通常几十到几百个.txt文件）。少量结构化素材（几本书、几篇访谈）不需要走这一步，直接读取即可。

**为什么需要**：原始ASR转录充斥大量噪音——打赏感谢、设备调试、结巴重复、极短废话。不清洗就让Agent读，相当于让Agent在垃圾堆里捡金子，效率极低且容易被噪音干扰判断。

**执行方式**：调用预处理脚本，在Agent读取前完成机械清洗。

```bash
python3 [女娲skill目录]/scripts/preprocess_corpus.py <素材目录> <输出目录>
```
...
| 步骤 | 做什么 | 方法 |
|------|--------|------|
| 1. 清洗 | ASR结巴修复（我我我觉得→我觉得）+ 规则过滤砍掉直播噪音（打赏、测试、极短废话） | 纯正则，零依赖 |
| 2. 去重 | 近似文本去重，剔除不同场次里反复说的同一段话 | MinHash（有datasketch时）或精确hash（降级） |
| 3. 提取 | 按长度筛选高价值段落（≥200字的完整论述） | 长度过滤 |

**输出三个文件**：
- `cleaned_corpus.txt`：清洗后全文，供Agent完整阅读
- `high_value_segments.txt`：高价值长段落，**优先给Agent看**——这些通常包含此人的完整论述和底层逻辑推演
- `preprocess_report.txt`：漏斗统计，用于Phase 1.5展示

**预处理后的Agent策略调整**：
- Agent 1（著作）和 Agent 2（对话）：优先读 `high_value_segments.txt`，信息不足再补 `cleaned_corpus.txt`
- Agent 3（表达DNA）：必须读 `cleaned_corpus.txt` 全文，因为短句碎片也是表达风格的一部分
- Agent 5（决策）：优先读 `high_value_segments.txt`，决策论述通常在长段落里

**不做的事**：
- 不做语义去重（需要GPU+embedding模型，环境依赖太重）
- 不做PPL困惑度过滤（同上）
- 不做意图规范化（女娲要保留原始表达DNA，不能把口语标准化）
...
#### 工具辅助（如可用）
- 大规模口语语料预处理：`python3 [女娲skill目录]/scripts/preprocess_corpus.py <素材目录> <输出目录>`（详见上方「大规模口语语料预处理」段落）
- 书籍：Z-Library/LibGen搜索下载 → 存入 `sources/books/`
- 视频字幕获取（已提供脚本，直接调用 — **如果脚本不存在则跳过，改用手动方式或其他已安装 skill**）：
...
- 调研摘要生成（Phase 1.5用）：`python3 [skill目录]/scripts/merge_research.py <skill目录>`（**如果脚本不存在，手动统计来源数和关键发现**）
  - 自动扫描 `references/research/01-06.md`，统计来源数、一手/二手占比、关键发现
  - 输出Phase 1.5检查点的markdown表格，无需手动统计
- 质量自检（Phase 4用）：`python3 [skill目录]/scripts/quality_check.py <SKILL.md路径>`（**如果脚本不存在，按下方6项标准手动逐项检查**）
...
6个Agent的素材汇总后，执行结构化提炼。如果 `references/extraction-framework.md` 存在则读取，获取心智模型的三重验证方法论；如果不存在，使用以下内联标准：**跨域复现**（在2+不同场景中出现过）、**生成力**（能推导出具体行动建议）、**自创术语**（人物独创的概念/命名），确保提炼质量。
...
2. **三重验证筛选**：对每个候选执行（如有 `references/extraction-framework.md` 则读取详细标准，否则按下方内联规则）：
   - 跨域复现：在≥2个不同领域/话题中出现？
   - 生成力：能推断此人对新问题的立场？
   - 排他性：不是所有聪明人都这样想？
...
#### Step 1: 读取模板
如果 `references/skill-template.md` 存在则读取获取标准结构；如果不存在，按以下骨架生成：frontmatter、角色扮演规则、身份卡、心智模型（含局限性）、决策启发式（含引用原话）、表达DNA、时间线、价值观、智识谱系、诚实边界、调研来源。参考已有的 kge-perspective 或 fengge-perspective 作为实际模板。
...
#### Step 3: 质量自检
构建完成后，执行质量自检。如果 `references/extraction-framework.md` 存在则读取末尾的「质量自检清单」；否则按以下6项逐项检查：(1)心智模型≥3个且每个有局限性 (2)决策启发式≥5条且有原话引用 (3)表达DNA有具体规则和示例 (4)诚实边界≥5条 (5)内在张力≥3对 (6)一手来源占比≥60%。不通过的项标注出来，回到对应Phase修复。
