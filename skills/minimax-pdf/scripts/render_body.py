...
def _contains_cjk(value) -> bool:
    """Return True when nested content contains CJK characters."""
...
def configure_cjk_fonts(tokens: dict, content: list):
    """Use ReportLab's built-in Simplified Chinese CID font for CJK documents."""
    if not _contains_cjk([tokens.get("title", ""), tokens.get("author", ""), content]):
        return
    ...
    # ReportLab's standard Helvetica/Times fonts do not cover Chinese. Use the
    # built-in CID font consistently so body text, headings, tables, headers, and
    # callouts render readable Chinese instead of tofu boxes.
    tokens["font_display_rl"] = "STSong-Light"
    tokens["font_body_rl"] = "STSong-Light"
    tokens["font_body_b_rl"] = "STSong-Light"
    tokens["_cjk"] = True
...
def build(tokens: dict, content: list, out_path: str) -> dict:
    ...
    configure_cjk_fonts(tokens, content)
