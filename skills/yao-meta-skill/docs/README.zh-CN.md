...
`YAO = Yielding AI Outcomes`，可以直接理解成：把 AI 结果变成能重复交付、能落地、能维护的真实成果。

`yao-meta-skill` 不是用来堆 prompt 的，而是用来把“已经反复验证过的工作流”打包成所有 agent 都能复用的 skill。它更适合做三件事：把规则说清、把边界锁死、把验收和回退做出来。

对你现在的直播拆解业务来说，它最有价值的不是“再写一段更长提示词”，而是把这条链路固化成一个正式 skill：

`raw_transcript.txt -> 连麦 / QA / 主动讲解 -> 跨分段合并 -> 质量验收 -> 失败回退 -> 飞书同步`
...
- 你每周都在拆直播，规则会反复迭代。
- 你希望所有 agent 用同一套口径。
- 你想把 6 月 8 号 L05 / L06、6 月 9 号空稿这类问题做成回归测试。
- 你不想让坏稿直接进飞书或知识库。

### 你要给它什么

1. 第一源头：`segment_*/raw_transcript.txt`
2. 硬规则：同一连麦必须合并、销售话术不能硬凑成连麦、QA / 主动讲解要分开
3. 成功样例：一两场拆对的稿
4. 失败样例：拆错、空稿、误分类样例
5. 输出契约：最终要落到哪个目录、什么 Markdown 结构、要不要同步飞书

### 它应该帮你产出什么

- `SKILL.md`：技能入口和使用说明
- `references/规则.md`：连麦 / QA / 主动讲解 / 回退规则
- `evals/回归样例.json`：6 月 8 号、6 月 9 号这种固定回归样例
- `scripts/验收脚本`：自动检查是否空稿、误拆、漏合并
- `reports/质量报告.md`：每次重构后看有没有退化

### 可以直接拿去用的提示词

```text
请把我这套“直播拆解”流程整理成一个可复用 skill。
输入是 `segment_*/raw_transcript.txt`，输出是结构化阅读稿和对应的 skill 资产。
硬规则：
1. 同一连麦跨分段必须合并
2. raw 是第一源头，不能只靠旧的 `qa_verbatim.md`
3. 连麦、QA、主动讲解必须分开
4. 销售话术、发福袋、领资料不能硬凑成连麦
5. 发现空稿、坏稿、误拆稿必须回退重跑
6. 飞书同步前必须先过验收
请先给我 skill 设计，再给我回归样例和验收点。
```
